...
const AIChatbot = ({ onAlertCreated, onResourceCreated }) => {
  ...
  const generateBotResponse = async (userMessage) => {
    ...
    if (
      ...
      lowerMessage.includes("my level")
    ) {
      if (!userId)
        return `You need to be logged in to check your Community Trust Score!`;
      try {
    ...
    // Weather information with real API call
    if (
      ...
      try {
        ...
        return `☁️ **Weather Update**\n\nWeather data unavailable right now. Please check a weather service directly!`;
      }
    }

    // Disaster trends with real data
    if (
      ...
      try {
        ...
        alerts.forEach((alert) => {
          ...
          else if (type.includes("medical")) counts.medical++;
          else counts.other++;
        });

        return `📈 **Disaster Trends (Last 50 alerts)**\n\n- Flood alerts: ${counts.flood}\n- Fire alerts: ${counts.fire}\n- Medical alerts: ${counts.medical}\n- Other: ${counts.other}\n\n**Total Active:** ${alerts.length} alerts`;
      } catch (err) {
    ...
    if (
      ...
      lowerMessage.includes("create alert") ||
      lowerMessage.includes("add alert")
    ) {
    ...
    if (
      lowerMessage.includes("add resource") ||
      lowerMessage.includes("share resource") ||
      lowerMessage.includes("offer resource")
    ) {
            ...
            try {
              ...
              if (alerts.length > 0) {
                const alertList = alerts
                  .slice(0, 3)
                  .map(
                    (a, i) => `${i + 1}. ${a.title} (Severity: ${a.severity})`,
                  )
                  .join("\n");
                resolve(
                ...
                resolve(
                  `📍 **Nearby Alerts**\n\nNo alerts found nearby. Your area seems safe!`,
                );
              }
            } catch (err) {
              resolve(
                `📍 **Nearby Alerts**\n\nCould not fetch nearby alerts right now. Check the map!`,
              );
            }
          },
          () => {
            resolve(
              `📍 **Nearby Alerts**\n\nI need location permission to find nearby alerts. Please enable geolocation.`,
            );
          },
        );
      });
  ...
  const handleQuickAlertSubmit = async (alertData) => {
    try {
      ...
      if (!alertData.confirmed) {
        const simRes = await mlAPI.getSimilarAlerts(
          alertData.latitude,
          alertData.longitude,
        ...
        <div className="grid grid-cols-2 gap-2">
          ...
          <div>
            ...
            >
              ...
              <option value="4">Power Outage</option>
