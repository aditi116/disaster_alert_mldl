...
const Dashboard = () => {
    ...
    if ('geolocation' in navigator) {
      ...
      navigator.geolocation.getCurrentPosition(
        (position) => {
          ...
          toast.success('Location found! Map centered on your position.', { id: 'location' });
          console.log('Location obtained:', latitude, longitude);
        },
        (error) => {
          console.warn('Geolocation error:', error.message);
          setLocationLoading(false);
          toast.error('Could not get your location. Using default map center.', { id: 'location' });
        },
        {
  ...
  const filterAlerts = () => {
    ...
    if (severityFilter !== 'all') {
      filtered = filtered.filter(alert => alert.severity === parseInt(severityFilter));
    }
  ...
  return (
    <div className="h-screen flex flex-col bg-gray-50 dark:bg-slate-900">
          ...
          <div className="flex items-center space-x-6">
            ...
            >
              ...
              <div>
                ...
                <p className="text-xs text-gray-500 dark:text-gray-400">Real-time Emergency Platform</p>
              </div>
            </motion.div>
            ...
            <div className="hidden lg:flex items-center space-x-3 pl-6 border-l border-gray-300 dark:border-slate-600">
              ...
              >
                ...
                <span className="text-sm font-bold text-red-600 dark:text-red-400">{stats.criticalAlerts}</span>
              ...
              >
                ...
                <span className="text-sm font-bold text-blue-600 dark:text-blue-400">{stats.activeAlerts}</span>
              ...
              >
                ...
                <span className="text-sm font-bold text-green-600 dark:text-green-400">{stats.totalResources}</span>
                <span className="text-xs text-gray-600 dark:text-gray-400">Resources</span>
              </motion.div>
            </div>
          </div>
            ...
            <div className="px-6 py-4">
                ...
                >
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-2xl font-bold text-green-600 dark:text-green-400">{stats.activeAlerts}</p>
                    </div>
                ...
                >
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-2xl font-bold text-purple-600 dark:text-purple-400">{stats.totalResources}</p>
                    </div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mt-4">
                ...
                <div className="bg-white dark:bg-slate-800 rounded-xl p-4 shadow-md border border-gray-200 dark:border-slate-700">
                  <p className="text-sm text-gray-500 dark:text-gray-400 mb-1">Total Votes</p>
                  <p className="text-xl font-bold text-gray-900 dark:text-white">{stats.totalVotes}</p>
                </div>

                {/* Average Severity */}
                <div className="bg-white dark:bg-slate-800 rounded-xl p-4 shadow-md border border-gray-200 dark:border-slate-700">
                  <p className="text-sm text-gray-500 dark:text-gray-400 mb-1">Avg Severity</p>
                  <p className="text-xl font-bold text-gray-900 dark:text-white">
                    {alerts.length > 0 
                      ? (alerts.reduce((sum, a) => sum + a.severity, 0) / alerts.length).toFixed(1)
                      : '0.0'
                    }
                  </p>
                </div>
                ...
                {/* Alert Types */}
                <div className="bg-white dark:bg-slate-800 rounded-xl p-4 shadow-md border border-gray-200 dark:border-slate-700">
                  <p className="text-sm text-gray-500 dark:text-gray-400 mb-1">Alert Types</p>
                  <p className="text-xl font-bold text-gray-900 dark:text-white">
          ...
          <div className="p-4 border-b border-gray-200 dark:border-slate-700 bg-gray-50 dark:bg-slate-900">
            <div className="relative">
              ...
              <button
                ...
                className={`absolute right-2 top-1/2 transform -translate-y-1/2 p-1 rounded ${
                  showFilters ? 'bg-blue-100 dark:bg-blue-900 text-blue-600' : 'text-gray-400 hover:text-gray-600'
                }`}
              >
            ...
            <AnimatePresence>
              {showFilters && (
                ...
                >
                  <label className="block text-xs font-medium text-gray-700 dark:text-gray-300">Severity Level</label>
                  <select
            ...
            {activeTab === 'alerts' ? (
              <>
                ...
                {loading ? (
                  ...
                  <div className="text-center py-8 text-gray-500 dark:text-gray-400">No alerts found</div>
                ) : (
              ...
              <>
                <h2 className="text-lg font-semibold text-gray-900 dark:text-white mb-4">Available Resources</h2>
                {loading ? (
                  <div className="text-center py-8 text-gray-500 dark:text-gray-400">Loading resources...</div>
                ) : filteredResources.length === 0 ? (
                  <div className="text-center py-8 text-gray-500 dark:text-gray-400">No resources found</div>
                ) : (
                  <div className="space-y-3">
                    {filteredResources.map((resource) => (
                      ...
                      >
                        ...
                        <p className="text-sm text-gray-600 dark:text-gray-300 mt-1 line-clamp-2">{resource.description}</p>
              ...
              >
                <Popup>
                  <div className="p-2">
                    ...
                    {resource.contactInfo && (
                      <p className="text-xs text-gray-500 mt-2">📞 {resource.contactInfo}</p>
                    )}
                    <button
                      onClick={() => {
                        setSelectedItem(resource);
      ...
      {/* Detail View Modal */}
      {selectedItem && (
        <div className="fixed inset-0 bg-black bg-opacity-50 dark:bg-opacity-70 flex items-center justify-center p-4" style={{ zIndex: 9999 }}>
          <motion.div
          ...
          >
            <div className="sticky top-0 bg-white dark:bg-slate-800 border-b border-gray-200 dark:border-slate-700 px-6 py-4 flex items-center justify-between rounded-t-2xl">
              <h2 className="text-xl font-bold text-gray-900 dark:text-white">
                {selectedItemType === 'alert' ? '🚨 Alert Details' : '📦 Resource Details'}
              </h2>
              <button
                onClick={() => {
                  setSelectedItem(null);
